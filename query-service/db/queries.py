        ...
        with conn.cursor() as cur:
            ...
            if start_time is None:
                ...
                if interval == 0:
                    ...
                    cur.execute("""
                        SELECT 
                            FROM_UNIXTIME((UNIX_TIMESTAMP(time) DIV %s) * %s) as time,
                            AVG(load_1m) as load_1m,
                            AVG(load_5m) as load_5m,
                            AVG(load_15m) as load_15m
                        FROM load_avg
                        WHERE sysname = %s AND time >= %s AND time <= %s
                        GROUP BY FROM_UNIXTIME((UNIX_TIMESTAMP(time) DIV %s) * %s)
                        ORDER BY time ASC
                    """, (interval, interval, sysname, start_time, end_time, interval, interval))
            ...
            if start_time is None:
                ...
                if interval == 0:
                    ...
                    cur.execute("""
                        ...
                        FROM cpu_percent
                        WHERE sysname = %s AND time >= %s AND time <= %s
                        GROUP BY cpu, FROM_UNIXTIME((UNIX_TIMESTAMP(time) DIV %s) * %s)
                        ORDER BY time ASC, cpu ASC
                    """, (interval, interval, sysname, start_time, end_time, interval, interval))
                result['cpu_percent'] = serialize_rows(cur.fetchall())
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            if start_time is not None:
                ...
                return list(reversed(rows))  # Reverse so time runs from oldest to newest
        ...
        with conn.cursor() as cur:
            if start_time is not None:
                ...
                if limit:
                    ...
                    """, (sysname,))
                rows = serialize_rows(cur.fetchall())
                return list(reversed(rows))  # Reverse so time runs from old to new
...
def get_network_metrics(
    ...
    """
    Network I/O metrics with rate calculation.

    Modes:
    - Snapshot mode (start_time is None):
        Return latest network counters + calculated rates (per interface).
    - Range mode:
        Return historical counters with calculated rates,
        optionally aggregated by safe, whitelisted interval.
    """
    with get_db() as conn:
        with conn.cursor() as cur:
            # ----------------------------
            ...
            if start_time is None:
                cur.execute(
                    ...
                    SELECT
                        ...
                        a.if_oper_status,
                        CASE
                            WHEN b.time IS NOT NULL
                             AND TIMESTAMPDIFF(MICROSECOND, b.time, a.time) > 0
                            THEN GREATEST(
                                0,
                    ...
                    FROM last2 a
                    LEFT JOIN last2 b
                        ON a.iface = b.iface AND a.rn = 1 AND b.rn = 2
                    WHERE a.rn = 1
                    ORDER BY a.iface
                    """,
                    (sysname,),
                    ...
                    SELECT
                        ...
                        CASE
                            WHEN prev_time IS NOT NULL
                             AND TIMESTAMPDIFF(MICROSECOND, prev_time, time) > 0
                            THEN GREATEST(
                                0,
            ...
            cur.execute(
                ...
                SELECT
                    ...
                    CASE
                        ...
                        THEN (MAX(bytes_sent) - MIN(bytes_sent)) / {interval}
                        ELSE NULL
                    END AS send_bytes_s,
                    CASE
                        WHEN MAX(bytes_recv) >= MIN(bytes_recv)
                        THEN (MAX(bytes_recv) - MIN(bytes_recv)) / {interval}
                        ELSE NULL
                    END AS recv_bytes_s
                FROM (
                    SELECT
...
def get_status_metrics(
    ...
    try:
        ...
        status.update(temperature_part or {})
